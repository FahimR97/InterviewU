...
function isCodingQuestion(q: Question): boolean {
  return (
    `${q.category} ${q.competency}`
      .toLowerCase()
      .match(/cod|programming|algorithm/) !== null
  );
}
...
function isSystemDesignQuestion(q: Question): boolean {
  return (
    `${q.category} ${q.competency}`
      .toLowerCase()
      .match(/system.design|architecture|design/) !== null
  );
}
...
const SD_TABS: { id: SdTab; label: string; hint: string }[] = [
  {
    id: "clarifying",
    label: "Clarify",
    hint: "What questions would you ask the interviewer before you start designing? (Users, scale, SLAs, read/write ratio, consistency…)",
  },
  {
    id: "requirements",
    label: "Requirements",
    hint: "Functional requirements (what it does) and non-functional requirements (scale, latency, availability, durability).",
  },
  {
    id: "design",
    label: "Architecture",
    hint: "High-level components and data flow. Core services, how they communicate, where the main data stores sit.",
  },
  {
    id: "deepdive",
    label: "Deep Dive",
    hint: "Pick the hardest component and go deeper — data model, API design, database choice, caching, queuing strategy.",
  },
  {
    id: "tradeoffs",
    label: "Tradeoffs",
    hint: "Why these choices? Failure modes and how you handle them. What would change at 10x scale?",
  },
];

function buildSdAnswer(s: SdSections): string {
  const parts: string[] = [];
  if (s.clarifying.trim())
    parts.push(`=== CLARIFYING QUESTIONS ===\n${s.clarifying.trim()}`);
  if (s.requirements.trim())
    parts.push(`=== REQUIREMENTS ===\n${s.requirements.trim()}`);
  if (s.design.trim())
    parts.push(`=== HIGH-LEVEL ARCHITECTURE ===\n${s.design.trim()}`);
  if (s.deepdive.trim()) parts.push(`=== DEEP DIVE ===\n${s.deepdive.trim()}`);
  if (s.tradeoffs.trim())
    parts.push(`=== TRADEOFFS & FAILURE MODES ===\n${s.tradeoffs.trim()}`);
  return parts.join("\n\n");
}
      ...
      <div className="practice-content">
            ...
            <div className="sd-context">
              ...
              <div className="sd-hints">
                {[
                  ...
                  <span key={h} className="sd-hint-tag">
                    {h}
                  </span>
                ))}
              </div>
        ...
        {!evaluation ? (
          <div className="practice-answer-panel">
            <p className="practice-panel-eyebrow">Your Answers</p>
              ...
              <div className="sd-answer">
                ...
                {SD_TABS.map((tab) =>
                  tab.id === "design" ? (
                    <div
                      key={tab.id}
                      className={`sd-section${sdTab === tab.id ? " active" : ""}`}
                    >
                      <p className="sd-section-hint">{tab.hint}</p>
                      <div
                        className={`sd-canvas-wrap${canvasFs ? " sd-fullscreen" : ""}`}
                      >
                        <button
                          className="sd-fs-btn"
                          onClick={() => setCanvasFs((f) => !f)}
                          title={canvasFs ? "Exit fullscreen" : "Expand canvas"}
                        >
                          {canvasFs ? "✕" : "⛶"}
                        </button>
                        <Suspense
                      ...
                      <p className="sd-canvas-label">
                        Describe your architecture for Marcus to evaluate
                        please:
                      </p>
                      <textarea
          ...
          <div className="practice-feedback">
            ...
            <div className="feedback-score-row">
              ...
              <div className="feedback-score-meta">
                ...
                <div
                  className={`feedback-verdict ${evaluation.score >= 80 ? "verdict-correct" : evaluation.score >= 60 ? "verdict-getting-there" : "verdict-improve"}`}
                >
                  {evaluation.score >= 80
...
export default function Questions() {
  ...
  const difficulties = useMemo(() => {
    const diffs = new Set(
      questions.map((q) => q.difficulty.toLowerCase()).filter(Boolean),
    );
    ...
    <div className="questions-container">
      ...
      <div className="qbank-hero">
        ...
        <p className="qbank-hero-subtitle">
          Curated interview questions across system design, DevOps, cloud,
          algorithms and more. Answer anything and get instant AI-scored
        ...
        <>
          ...
          {error && (
            <div className="error-box">
              ...
              <button className="btn-refresh-inline" onClick={loadQuestions}>
                Retry
              </button>
            </div>
          )}
